...
void AudioConfigPane::InitializeGUI()
{
  ...
  m_dsp_engine_strings.Add(_("DSP LLE IR Recompiler"));
...
void AudioConfigPane::LoadGUIValues()
{
  ...
  if (startup_params.bDSPHLE)
    ...
    m_dsp_engine_radiobox->SetSelection(SConfig::GetInstance().m_DSPEnableJIT ?
                                            (SConfig::GetInstance().m_DSPEnableJITIR ? 2 : 1) :
                                            3);
...
void AudioConfigPane::OnDSPEngineRadioBoxChanged(wxCommandEvent& event)
{
  ...
  SConfig::GetInstance().m_DSPEnableJIT =
      m_dsp_engine_radiobox->GetSelection() == 1 || m_dsp_engine_radiobox->GetSelection() == 2;
  SConfig::GetInstance().m_DSPEnableJITIR = m_dsp_engine_radiobox->GetSelection() == 2;
